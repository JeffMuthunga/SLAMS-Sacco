...
- [x] Phase 12: Member portal — MemberPortalController (/me/* routes, auth:sanctum scoped to member record), 10 member-scoped endpoints (dashboard, profile, accounts+statement, loans, contributions, guarantees, issues+comments, transactions, petty-cash); frontend: dashboard with stats, my-details, guarantees (requests/active/closed), petty-cash (allocations/requests), service-desk (accounts with inline statement, loans with schedule, issues with inline create+comment, transfers, commodities placeholder), account-statement (transactions, contributions) (2026-06-12)
- [x] Phase 13: Reports — ReportController (7 endpoints: members, loans, contributions, accounts, transactions, issues, petty-cash; each with date/status filters + summary totals in meta); frontend: /admin/reports/sacco (tabbed: Members/Loans/Contributions/Savings Accounts with filter panels + summary cards + DataTable export), /admin/reports/operations (tabbed: Transactions/Petty Cash/Issues) (2026-06-12)
- [x] Phase 14: SMS & email notifications — SmsService (Africa's Talking, phone normalisation, disabled-by-default), SendSmsJob + SendEmailJob (queued, 3 retries), NotificationService (member/loan/petty-cash/issue events, htmlWrap); wired into MemberService, LoanService, PettyCashService, IssueService; .env.example updated with SMS vars (2026-06-12)
