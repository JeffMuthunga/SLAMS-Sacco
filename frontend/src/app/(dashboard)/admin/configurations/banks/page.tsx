...
export default function BanksPage() {
  const { data: items, isLoading } = useBanks();
  ...
  const updateMutation = useUpdateBank(editingId ?? "");
  ...
  const handleDelete = (id: string) => {
    if (!confirm("Are you sure you want to archive this bank?")) return;
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">
            Manage banks associated with SACCO accounts.
          </p>
        </div>
        <button
          onClick={() => {
