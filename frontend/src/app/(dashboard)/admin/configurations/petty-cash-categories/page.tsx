...
export default function PettyCashCategoriesPage() {
  const { data: items, isLoading } = usePettyCashCategories();
  const createMutation = useCreatePettyCashCategory();
  const updateMutation = useUpdatePettyCashCategory();
  const deleteMutation = useDeletePettyCashCategory();
  ...
  const handleEdit = (item: PettyCashCategory) => {
  ...
  return (
    <div className="space-y-6 p-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold text-dark dark:text-white">Petty Cash Categories</h1>
          <p className="mt-1 text-sm text-gray-500">Manage categories for petty cash items and requests.</p>
        </div>
        <button
            ...
            {items?.map((item) => (
              <tr key={item.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50">
                ...
                <td className="px-4 py-3 text-sm text-gray-500">
                  {new Date(item.created_at).toLocaleDateString("en-BW")}
                </td>
