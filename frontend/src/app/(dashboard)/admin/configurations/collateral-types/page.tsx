...
import {
  CollateralType,
  CreateCollateralTypePayload,
  useCollateralTypes,
  useCreateCollateralType,
  useUpdateCollateralType,
  useDeleteCollateralType,
...
const defaultForm: FormData = { name: "", description: "", is_active: true };
...
export default function CollateralTypesPage() {
  const { data: items, isLoading } = useCollateralTypes();
  const createMutation = useCreateCollateralType();
  const deleteMutation = useDeleteCollateralType();

  const [showForm, setShowForm] = useState(false);
  const [editingId, setEditingId] = useState<string | null>(null);
  const [formData, setFormData] = useState<FormData>(defaultForm);

  const updateMutation = useUpdateCollateralType(editingId ?? "");
  ...
  const handleEdit = (item: CollateralType) => {
    setFormData({
      name: item.name,
  ...
  const handleDelete = (id: string) => {
    if (!confirm("Are you sure you want to archive this collateral type?")) return;
    deleteMutation.mutate(id, {
      onSuccess: () => toast.success("Collateral type archived."),
  ...
  return (
    <div className="space-y-6 p-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold text-gray-900">Collateral Types</h1>
          <p className="mt-1 text-sm text-gray-500">
            Manage the types of collateral accepted for loans.
          </p>
        </div>
        <button
          onClick={() => {
      ...
      {showForm && (
        <div className="rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
          <h2 className="mb-4 text-base font-semibold text-gray-900">
            {editingId ? "Edit Collateral Type" : "New Collateral Type"}
          </h2>
